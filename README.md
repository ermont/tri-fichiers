...
[1](#fig:structure){reference-type="ref" reference="fig:structure"}, est
très simple : le sous-programme `chercherFichiers` effectue une
...
<figcaption>Structure de notre programme de tri.</figcaption>
</figure>
...
Compilez le programme à l'aide du fichier `Makefile` et exécutez le :
...
-   ou bien :\
    `int pthread_mutex_init(pthread_mutex_t *verrou, const pthread_mutexattr_t *mutexattr);`
...
::: {#tab:eval}
  ...
  : Temps d'exécution de `trier` et `trier-mt` pour 3 répertoires
  différents
:::
